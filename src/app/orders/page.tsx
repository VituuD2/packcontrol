      ...
      <div className="rounded-[1.25rem] border-0 dark:border dark:border-border shadow-[0_2px_20px_rgb(0,0,0,0.04)] dark:shadow-none bg-white dark:bg-card overflow-hidden">
        {orders && orders.length > 0 ? (
            ...
            {orders.map((order) => {
              ...
              return (
                <div key={order.id} className="flex flex-col sm:flex-row items-center p-4 gap-4 hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                  ...
                  {/* Order Number */}
                  <div className="w-24 pl-2 font-mono font-semibold text-foreground/80 dark:text-[#E2E2E2]">
                    #{orderNumber}
                  </div>

                  {/* Customer Info */}
                  <div className="flex-1 min-w-[200px] flex flex-col justify-center">
                    <span className="font-semibold text-[15px] truncate text-foreground/90 dark:text-[#EDEDED]">{customerName}</span>
                    <span className="text-xs font-medium text-muted-foreground truncate">{payload.billing?.email}</span>
                  </div>

                  {/* Total */}
                  <div className="w-32 text-right pr-4">
                    <span className="font-bold text-[15px] text-foreground/80 dark:text-[#E2E2E2]">{total}</span>
                  </div>

                  {/* Status / SKUs */}
                  <div className="w-48 flex justify-center items-center gap-2 sm:border-l border-gray-100 dark:border-white/[0.04]">
                    {isProcessed ? (
                      <Badge className="bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 border-0 h-6 px-2 text-[11px] font-bold tracking-wide rounded-md gap-1">
                        <CheckCircle2 className="w-3 h-3" /> {t('orders.processed')}
                      </Badge>
                    ) : (
                      <Badge className="bg-amber-50 text-amber-500 dark:bg-amber-500/10 dark:text-amber-400 border-0 h-6 px-2 text-[11px] font-bold tracking-wide rounded-md gap-1">
                  ...
                  {payload.line_items && Array.isArray(payload.line_items) && (
                    <div className="hidden lg:flex w-64 flex-wrap gap-1 items-center px-4 sm:border-l border-gray-100 dark:border-white/[0.04]">
                      {payload.line_items.slice(0, 2).map((li: any, idx: number) => (
                        <span key={idx} className="text-[10px] font-mono text-muted-foreground bg-gray-50 dark:bg-white/[0.04] px-1.5 py-0.5 rounded border border-gray-100/50 max-w-full truncate">
                  ...
                  <div className="w-12 flex justify-end pr-2">
                     {!isProcessed && (
                       <Link href="/products" className="p-2 rounded-lg bg-indigo-50 text-indigo-500 dark:bg-indigo-500/10 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-colors" title="Fix Missing Recipe in Products">
          ...
          <div className="p-16 flex flex-col items-center justify-center text-center">
            ...
            <h2 className="text-[15px] font-semibold text-foreground/80 dark:text-[#E2E2E2] tracking-tight">{t('orders.no_orders')}</h2>
